function movie
% Show a movie in which a particular iron plate conducts heat in 2D
    % Initialization
    kappa = 48; % 熱伝導率
    c = 461; % 比熱
    rho = 7.28; % 密度
    ...
    X = linspace(0, L, n+1); % x軸における空間の区切り目
    Y = linspace(0, L, n+1); % y軸における空間の区切り目
    mesh = zeros(n+1, n+1); % 空間のメッシュ
    
    v = VideoWriter('heatconduction.avi');
    open(v);
    hfig = figure ();
